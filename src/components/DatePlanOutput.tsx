...
const DatePlanOutput: React.FC<DatePlanOutputProps> = ({ profileData, completeDatePlan }) => {
  ...
  const datePlan = activities.length > 0 ? activities.map((activity: any, index: number) => ({
    time: activity.time || `${14 + index * 2}:00`, // Default times if not provided
    title: activity.title || activity.name || "Activity",
    location: activity.location || "Location TBD",
    reason: activity.description || activity.what_to_do || "Perfect for your personalities",
    spark: activity.conversation_topics?.[0] || "What's your favorite memory from a place like this?",
    backup: activity.backup_option || "Weather backup available",
  ...
  const generateWhatsAppText = () => {
    const whatsappText = `🌟 *Our AI Date Plan* 🌟

📍 ${profileData.context?.location || 'Your City'}
⏰ ${profileData.context?.duration || '4'} hours | 🌤️ ${profileData.context?.time_of_day || 'evening'}
...
Generated by AI-more.me ✨`;
  ...
  const saveAsImage = () => {
    ...
    ctx.fillText('🌟 Your AI Date Plan', 400, 60);
    
    ctx.font = '18px Arial';
    ctx.fillText(`📍 ${profileData.context?.location || 'Your City'}`, 400, 100);
    ctx.fillText(`⏰ ${profileData.context?.duration || '4'} hours | 🌤️ ${profileData.context?.time_of_day || 'evening'}`, 400, 130);
    ...
    ctx.font = '14px Arial';
    ctx.fillText('Generated by AI-more.me ✨', 400, yPos + 100);
  ...
  const downloadHTML = () => {
    ...
    const pdfContent = `
      <html>
        <head>
          <title>AI-more.me Date Plan</title>
          <style>
        ...
        <body>
          <div class="header">
            <h1>🌟 Your AI Date Plan</h1>
            <p><strong>Location:</strong> ${profileData.context?.location || 'Your City'}</p>
            <p><strong>Duration:</strong> ${profileData.context?.duration || '4'} hours | <strong>Time:</strong> ${profileData.context?.time_of_day || 'evening'}</p>
          </div>
          ...
          <div style="text-align: center; margin-top: 30px;">
            ...
            <p>Based on compatibility analysis</p>
            <br>
            <p style="color: #666; font-size: 12px;">Generated by AI-more.me - The Tinder of Date Planning</p>
          </div>
        </body>
      </html>
    `;
              ...
              <div className="space-y-2">
                {apiDatePlan?.reasoning?.success_prediction && (
                  <p className="text-white/80 flex items-start gap-2">
                    ...
                    Success probability: {successProbability}% based on compatibility analysis
                  </p>
                )}
                <p className="text-white/80 flex items-start gap-2">
                  <span className="text-green-400">✓</span>
                ...
                {completeDatePlan?.cultural_intelligence_summary && (
                  <p className="text-white/80 flex items-start gap-2">
                    ...
                    Based on {completeDatePlan.cultural_intelligence_summary.total_discoveries_analyzed || 69} compatibility factors analyzed
                  </p>
                )}
                <p className="text-white/80 flex items-start gap-2">
                  <span className="text-blue-400">ℹ</span>
                  Powered by cultural intelligence and real venue data
                </p>
              </div>
            </div>
          </motion.div>
